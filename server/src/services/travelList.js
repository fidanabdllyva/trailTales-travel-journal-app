...
const getAll = async () => await TravelListModel.find()
...
const getById = async (listId, userId) => {
    ...
    const hasAccess = list.isPublic ||
                     list.owner._id.toString() === userId.toString() ||
                        list.collaborators.some(collab => collab._id.toString() === userId.toString());
...
const getUserLists = async (userId) => {
    ...
    const getCollaborativeLists = async (userId) => {
    ...
    const create = async (payload, userId) => {
        try {
            ...
            const newList = await TravelListModel.create({
                ...
                owner: userId,
                coverImage: coverImage || "https://reactplay.io/static/media/placeholder_cover.ea7b18e0704561775829.jpg",
                collaborators: [],
                destinations: [],
            });
    ...
    const update = async (listId, payload, userId) => {
        try {
            const list = await TravelListModel.findById(listId);
    ...
    const addDestination = async (listId, destinationId, userId) => {
        try {
            const list = await TravelListModel.findById(listId);
    ...
    const removeDestination = async (listId, destinationId, userId) => {
        try {
            const list = await TravelListModel.findById(listId);
