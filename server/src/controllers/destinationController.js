...
module.exports = {
  ...
  async getDestinationsByList(req, res) {
    try {
      const destinations = await destinationService.getDestinationsByList(
        req.params.listId,
        req.query.status,
        req.user.id
      );
  ...
  async removeImage(req, res) {
    try {
      // no need for imageId → only one image in schema
      const destination = await destinationService.removeImage(
        req.params.id,
        req.user.id
      );
      res.json({
